    ...
    return BlocConsumer<DetailsCubit, DetailsState>(
      ...
      builder: (context, state) {
        ...
        if (state is GetDetailsSuccessState ||
            state is DetailsTaskDeletingState) {
                ...
                PopupMenuButton(
                  itemBuilder: (context) => [
                    PopupMenuItem(
                      child: TextButton(
                        onPressed: () async {
                          final result = await Navigator.pushNamed(context, RouteGenerator.editTask,
                              arguments: task);
                          if (result == true) {
                            context.read<DetailsCubit>().getTask(widget.taskId); // Refresh task details
                          }
                        },
          ...
          actions: [
            ...
            TextButton(
              ...
              child: Text(
                "Sure",
                style: TextStyle(
                    fontSize: 14, color: AppColors.inprogressTextColor),
              ),
            ),
          ],
        );
